  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (p.indexOf('/api/start-task') >= 0) {
        ...
        ctx.waitUntil(runAgg(tid, env));
        return jresp({ok: true, tid: tid});
      }
      if (p.indexOf('/api/status') >= 0) {
        var tid2 = u.searchParams.get('tid');
        if (!tid2) return jresp({error: 'no tid'}, 400);
        if (!env.TVBOX_KV) return jresp({error: 'no KV'}, 500);
        var raw = await env.TVBOX_KV.get(tid2);
        if (!raw) return jresp({status: 'pending', logs: '...'});
        return jresp(JSON.parse(raw));
      }
      if (p.indexOf('/subscribe.json') >= 0) {
        if (!env.TVBOX_KV) return jresp({note: 'no KV'}, 500);
        var data = await env.TVBOX_KV.get('latest_aggregated_result');
        if (!data) return jresp({note: 'no data'}, 404);
        return new Response(data, {headers: {'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});
      }
    } catch(e) {
      return jresp({error: e.message}, 500);
    }
