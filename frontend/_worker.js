// TVBox Aggregator Worker - Fixed V3
...
async function runAggregation(taskId, env) {
    ...
    try {
        ...
        if (!ghToken) throw new Error("配置错误: 未找到 GH_TOKEN。请在 Cloudflare 设置中添加环境变量。");
        ...
        if (!searchResponse.ok) throw new Error('GitHub API 搜索失败: ' + searchResponse.status + '. 可能 Token 无效或 API 限流。');
